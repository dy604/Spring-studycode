...
public abstract class ClassUtils {
    ...
    public static ClassLoader getDefaultClassLoader() {
        ClassLoader cl = null;
        try {
            cl = Thread.currentThread().getContextClassLoader(); //得到线程上下文类加载器
        } catch (Throwable ex) {
            //如果不能得到线程上下文类加载器，回落使用系统类加载器
        }
        if (cl == null) {
            cl = ClassUtils.class.getClassLoader(); //如果没有得到线程上下文类加载器则使用这个类的类加载器
        }
        return cl;
    }
    ...
    public static ClassLoader overrideThreadContextClassLoader(ClassLoader classLoaderToUse) {
        ...
        ClassLoader threadContextClassLoader = currentThread.getContextClassLoader(); //得到当前线程的上下文类加载器
        if (classLoaderToUse != null && !classLoaderToUse.equals(threadContextClassLoader)) {
            currentThread.setContextClassLoader(classLoaderToUse); //将传进来的类加载器设置给当前线程作为线程上下文类加载器
    ...
    public static Class<?> getUserClass(Class<?> clazz) {
        if (clazz != null && clazz.getName().contains(CGLIB_CLASS_SEPARATOR)) {
            Class<?> superClass = clazz.getSuperclass(); //获得此类的父类对应的Class对象
    ...
    public static String getShortNameAsProperty(Class<?> clazz) {
        ...
        //获得一个字符串并将它转换成普通java变量名称，即将变量首字母小写，其他部分不变，
        //如User类变为user，如BeanFactoryTest变为beanFactoryTest
    ...
    public static Method getMostSpecificMethod(Method method, Class<?> targetClass) {
        if (method != null && isOverrideable(method, targetClass) &&
    ...
    public static boolean isOverrideable(Method method, Class targetClass) {
    ...
    public static boolean isPrimitiveWrapper(Class<?> clazz) {
        Assert.notNull(clazz, "Class must not ne null");
